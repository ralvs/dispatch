...
function systemPrompt(ctx: ParseContext): string {
	return [
		...
		"- log_health_metric { metric, value?, value_secondary?, unit? } — a",
		"  health/vitals reading (weight, blood pressure, sleep, steps…). Prefer",
		"  canonical metric names when obvious (weight, bp, hr_resting,",
		"  sleep_duration, steps). For bp use value=systolic, value_secondary=diastolic.",
