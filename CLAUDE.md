...
Single-user personal operations dashboard PWA. Voice-first capture, editorial
...
4. **Never lose a capture.** The voice pipeline degrades to a `needs_review`
   note rather than dropping input. AI calls return typed fallbacks, never
   throw into the capture path.
5. **Bilingual PT-BR/EN.** Content is stored verbatim in the language spoken —
