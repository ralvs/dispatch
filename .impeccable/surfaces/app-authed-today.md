...
`_a.css` and proved in `.impeccable/mocks/palette-lab.html`. The incumbent
...
or "overdue" — and Engine is the most frequent domain. engine/travel and
code/finance were also indistinguishable at dot size.

**Storage changes shape.** A stored hex cannot theme-switch, and dark is a full
peer now. `stewardship_domains.color` becomes a **palette token slug**
(`"health"`), resolved to a light/dark pair in CSS. Retuning a colour becomes a
token edit rather than an UPDATE, and the picker offers the nine, not free hex.
